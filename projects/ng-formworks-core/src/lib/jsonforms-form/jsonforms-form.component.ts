import { Component, inject, input, model as modelSignal, OnInit, output } from '@angular/core';
import { createAjv } from '@jsonforms/core';
import { cloneDeep, isEmpty } from 'lodash';
import { FrameworkLibraryService } from '../framework-library/framework-library.service';
import { coreRenderers } from '../jsonforms-renderers';
import { convertSchemaToDraft6, hasOwn } from '../shared';
import { hasValue, inArray, isObject } from '../shared/validator.functions';
@Component({
...
export class JsonformsFormComponent implements OnInit{
  ...
  readonly data = modelSignal<any>(undefined); // The form data
  readonly schema = modelSignal<any>(undefined); // The schema
  readonly uischema = modelSignal<any>(undefined); // The uischema
  readonly framework = input<any | string>(undefined); // The framework to load
  readonly renderers = modelSignal<any>(undefined); // Any custom renderers to load
  ...
  constructor(){
    ...
    ngOnInit() {
      ...
      let jsonformsSchema;
      if (isObject(schema)) {
        jsonformsSchema = cloneDeep(schema);
      } else if (hasOwn(form, 'schema') && isObject(form.schema)) {
        jsonformsSchema = cloneDeep(form.schema);
      } else if (hasOwn(form, 'properties') && isObject(form.properties)) {
        jsonformsSchema = cloneDeep(form);
      } else if (isObject(form)) {
        // TODO: Handle other types of form input
      }
  
      if (!isEmpty(jsonformsSchema)) {
  
        // If other types also allowed, render schema as an object
        if (inArray('object', jsonformsSchema.type)) {
          jsonformsSchema.type = 'object';
        }
  
        // Wrap non-object schemas in object.
        if (hasOwn(jsonformsSchema, 'type') && jsonformsSchema.type !== 'object') {
          jsonformsSchema = {
            'type': 'object',
            'properties': { 1: jsonformsSchema }
          };
          //this.objectWrap = true;
        } else if (!hasOwn(jsonformsSchema, 'type')) {
  
          // Add type = 'object' if missing
          if (isObject(jsonformsSchema.properties) ||
            isObject(jsonformsSchema.patternProperties) ||
            isObject(jsonformsSchema.additionalProperties)
          ) {
            jsonformsSchema.type = 'object';
        ...
        jsonformsSchema = convertSchemaToDraft6(jsonformsSchema);
       ...
       /*
        jsonformsSchema = resolveSchemaReferences(
          jsonformsSchema, this.jsf.schemaRefLibrary, this.jsf.schemaRecursiveRefMap,
        ...
        */
        ...
        //   ));
        this.schema.set(jsonformsSchema);
      }
    }
         ...
         private initializeData() {
            ...
            let jsonformsData;
            if (hasValue(data)) {
              jsonformsData= cloneDeep(data);
            } 
            // else if (hasValue(model)) {
            //   jsonformsData = cloneDeep(model);
            // } else if (hasValue(ngModel)) {
            //   jsonformsData = cloneDeep(ngModel);
            // } 
            else if (isObject(form) && hasValue(form.value)) {
              jsonformsData = cloneDeep(form.value);
            } else if (isObject(form) && hasValue(form.data)) {
              jsonformsData = cloneDeep(form.data);

            } else if (hasOwn(form, 'formData') && hasValue(form.formData)) {

              jsonformsData = cloneDeep(form.formData);
            } 
            this.data.set(jsonformsData);
          }
...
/*
  ...
  */
    ...
    private initializeLayout() {
      ...
      //const model = this.model();
      //const ngModel = this.ngModel();
      let jsonformsUISchema;
      if (hasValue(uischema)) {
        jsonformsUISchema= cloneDeep(uischema);
      } else if (isObject(form) && hasValue(form.JFUISchema)) {
        jsonformsUISchema = cloneDeep(form.JFUISchema);
      } 
      this.uischema.set(jsonformsUISchema);
